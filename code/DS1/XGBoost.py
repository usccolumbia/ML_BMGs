...
def main():
    ...
    # create the dataset
    df_xgb = df.copy().drop(columns=["ID", "Ref.", "Cit.", "Alloy", "Fe"])
    target, features = df_xgb.iloc[:, :1], df_xgb.iloc[:, 1:]
    ...
    X = df_xgb.copy().drop(columns=["Dmax"], axis=1)
    y = target.copy()
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=True, random_state=81)
    xgb_shap = xgb.XGBRegressor(**optimal_hyperparameters)
    xgb_shap.fit(X_train, y_train)
    explainer = shap.TreeExplainer(xgb_shap)
    shap_values2 = explainer.shap_values(X_test)
    fig_2 = shap.summary_plot(shap_values2, X_test, max_display=10, plot_size=[10, 5], show=False)
