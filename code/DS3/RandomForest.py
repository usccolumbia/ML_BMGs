...
def feature_importance_random_forest(model, features):
...
def main():
    ...
    model_rf = RandomForestRegressor(**optimal_hyperparameters)
    score_grid_r2 = np.round(
        pd.DataFrame(cross_validate(model_rf, features, np.array(target).ravel(), cv=cv, return_train_score=True))
        .drop(columns=["fit_time", "score_time"]), 2)
    score_grid_mae = np.round(pd.DataFrame(cross_validate(model_rf, features, np.array(target).ravel(),
    ...
    print(50 * "-")
    model_rf.fit(features_train, np.array(targets_train).ravel())
    y_pred = model_rf.predict(features)
    ...
    feature_importance_random_forest(model_rf, features)
    shap_values_plots(model_rf, features_test)
