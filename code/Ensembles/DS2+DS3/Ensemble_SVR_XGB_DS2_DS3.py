...
def learning_curve_model(model, features, target, cv):
    train_size, train_scores, valid_scores = learning_curve(model, features, target,
                                                            train_sizes=np.linspace(0.25, 1, 50), scoring="r2",
                                                            shuffle=True, random_state=81, cv=cv, n_jobs=-1)
    train_errors = train_scores.mean(axis=1)
    valid_errors = valid_scores.mean(axis=1)
    train_std = train_scores.std(axis=1)
    valid_std = valid_scores.std(axis=1)
    plt.figure(figsize=(6, 4))
    plt.plot(train_size, train_errors, "bo-", linewidth=2, label="train")
    plt.plot(train_size, valid_errors, "ro-", linewidth=3, label="valid")
    plt.fill_between(train_size, train_errors + train_std, train_errors - train_std, alpha=0.15, color="blue")
    plt.fill_between(train_size, valid_errors + valid_std, valid_errors - valid_std, alpha=0.15, color="green")
    plt.xlabel("Train size")
    plt.axhline(y=0.68, color="r", linestyle="--", linewidth=0.75, label="Predicted R2")
    plt.ylabel("R2")
    plt.grid()
    plt.legend(loc="upper right", )
    plt.axis([-0.1, 495, -0.1, 1.1])

    plt.show()


def main():
    df_original = pd.read_csv(r"C:\Users\39366\Desktop\Modelli Finali\Ensemble\DS2+DS3\data_ensemble.csv")
    df = df_original.drop(columns=["ID", "Alloy","Cit.", "Ref.", "Fe", "T0 phase", "DGM phase Tsol", "DGM phase T0"])
    ...
    pipe_xgb_magpie = Pipeline([
        ("col_extract", ColumnExtractor(cols=range(36, 181))),
        ("xgb", xgb.XGBRegressor(max_depth=8, n_estimators=100, min_child_weight=2, learning_rate=0.1,
                                 gamma=0.05, subsample=0.5))
        ])
    pipe_svm_magpie = Pipeline([
        ("col_extract", ColumnExtractor(cols=range(36, 181))),
        ("SVM", make_pipeline(StandardScaler(), SVR(C=10, epsilon=0.1, gamma=0.03)))
        ])
    print(50*"-")
    estimators = [("SVM(0)", pipe_svm_0), ("XGBoost(0)", pipe_xgb_0), ("SVM(1)", pipe_svm_1),
                  ("XGBoost(1)", pipe_xgb_1), ("XGBoost magpie", pipe_xgb_magpie), ("SVM magpie", pipe_svm_magpie)]
    ...
    vr = VotingRegressor(estimators, weights=[0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
    ...
    learning_curve_model(vr, features_ensemble, target_ensemble, cv)
