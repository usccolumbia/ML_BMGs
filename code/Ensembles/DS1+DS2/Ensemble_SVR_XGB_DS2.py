...
def main():
    ...
    df_ensemble_1 = df.copy().drop(columns=["ID", "Ref.", "Cit.", "Alloy", "Fe", "Co", "B", "Si", "Nb", "Ni", "Y", "Zr",
                                            "Cr", "Mo", "P", "C", "Hf", "Al", "Dy", "Er", "Mn", "Ti", "V", "Cu", "Sn",
                                            "W", "Ta", "T0 phase", "DGM phase Tsol", "DGM phase T0"], axis=1)

    features_ensemble = df_ensemble_1.drop("Dmax", axis=1)  # work on all dataset
    target_ensemble = df_ensemble_1["Dmax"].copy()

    cv = KFold(n_splits=10, shuffle=True, random_state=18)
    svr_grid_final = make_pipeline(StandardScaler(),
                                   SVR(C=5, epsilon=0.01, gamma=1))
    xgb_model_grid = xgb.XGBRegressor(max_depth=8, n_estimators=100, min_child_weight=5, learning_rate=0.1,
