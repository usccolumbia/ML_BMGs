...
def main():
    ...
    df.drop(columns=["ID", "Ref.", "Cit.", "Alloy", "T0 phase", "DGM phase Tsol", "DGM phase T0"], inplace=True)

    # Split the data into features and target
    features = df.drop("Dmax", axis=1)  # work on all dataset
    target = df["Dmax"].copy()

    features_train, features_test, targets_train, targets_test = train_test_split(features, target,
                                                                                  test_size=0.2, shuffle=True,
                                                                                  random_state=81)
    # Perform cross-validation to evaluate default SVR model
    ...
    svr_grid_final = make_pipeline(StandardScaler(), SVR(**optimal_hyperparameters))
    ...
    r2_final_grid_1 = np.round(score_grid_r2["train_score"].mean(), 3)
    ...
    shap.summary_plot(shap_values, features_test_shap, max_display=10,
                      feature_names=list_columns, plot_type="bar", color="#ff0d57", show=False)
